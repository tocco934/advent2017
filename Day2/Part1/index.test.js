const checksumCalculator = require('./').calculateChecksums;

test('Returns 0 if given no rows', () => {
  expect(checksumCalculator([])).toBe(0);
});

test('Returns 0 if given a single value', () => {
  expect(checksumCalculator([[1]])).toBe(0);
});

test('Returns 8 given the sequence 5195', () => {
  expect(checksumCalculator([[5, 1, 9, 5]])).toBe(8);
});

test('Handles multiple rows', () => {
  const input = [[5, 1, 9, 5], [7, 5, 3], [2, 4, 6, 8]];
  expect(checksumCalculator(input)).toBe(18);
});

test('Calculates the puzzle\'s solution', () => {
  const input = [
  ...
  expect(checksumCalculator(input)).toBe(51139);
});
